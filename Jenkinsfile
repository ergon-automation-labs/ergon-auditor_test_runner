pipeline {
  ...
  environment {
    BOT_NAME = 'test_runner_bot'
    RELEASE_DIR = "/opt/ergon/releases/${BOT_NAME}"
    GITHUB_REPO = "ergon-automation-labs/ergon-auditor_test_runner"
  }
    ...
    stage('Run Migrations') {
      steps {
        sh '''
          ...
          RELEASE_BIN="${RELEASE_DIR}/current/test_runner_bot/bin/test_runner_bot"
          ...
          echo "Running: $RELEASE_BIN eval 'BotArmyAuditorTestRunner.Release.create()'"
          $RELEASE_BIN eval 'BotArmyAuditorTestRunner.Release.create()'

          echo "Running: $RELEASE_BIN eval 'BotArmyAuditorTestRunner.Release.migrate()'"
          $RELEASE_BIN eval 'BotArmyAuditorTestRunner.Release.migrate()'
  ...
  post {
    success {
      sh '''
        ...
        if [ -f ./release-artifact/bot_army_auditor_test_runner/releases/start_erl.data ]; then
          VERSION=$(awk '{print $2}' ./release-artifact/bot_army_auditor_test_runner/releases/start_erl.data)
        fi
